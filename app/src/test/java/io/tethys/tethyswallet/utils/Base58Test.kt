...
class Base58Test {
    ...
    fun encodeToBase58() {
        TEST_VECTORS.forEach {
            assertThat(Hex.decode(it.key.toByteArray()).encodeToBase58String(), equalTo(it.value))
        }
    }

    @Test
    fun decodeFromBase58() {
        TEST_VECTORS.forEach {
            assertThat(it.value.decodeBase58(), equalTo(Hex.decode(it.key)))
        }
    }
}